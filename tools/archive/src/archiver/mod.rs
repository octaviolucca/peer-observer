...
pub struct Args {
    ...
    /// Requires --messages and cannot be combined with other event filters.
...
mod tests {
    ...
    #[test]
    fn low_data_requires_messages_and_conflicts_with_other_filters() {
        assert!(parse_args(&["--low-data"]).is_err());
        assert!(parse_args(&["--low-data", "--messages"]).is_ok());
        assert!(parse_args(&["--low-data", "--messages", "--rpc"]).is_err());
    }
