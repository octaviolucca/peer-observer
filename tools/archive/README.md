...
wtxid. `block` messages keep only their header by default, including the block hash, and drop all
block transactions.
...
inspect the transactions and blocks themselves. Other event types are archived unchanged when their
filters are enabled outside low-data mode. Low-data mode requires `--messages` and cannot be
combined with other event filters.
...
Options:
  ...
  -l, --log-level <LOG_LEVEL>
      ...
      --low-data
          If passed, don't archive raw transaction data. Requires --messages and cannot be combined with other event filters. Transactions keep their txid and wtxid. Blocks keep only their header
  -h, --help
          Print help
  -V, --version
          Print version
